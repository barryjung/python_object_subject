...
class Barista(Person):
    ...
    def action(self, customer):
        ...
        """
        for ingredient, quantity in customer.order.ingredients.items():
            print("여기", ingredient, quantity)
            self.inventory.remove_item_quantity(ingredient, quantity)
        else:
